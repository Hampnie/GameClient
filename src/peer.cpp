...
Peer::Peer(boost::asio::io_context *context, std::string ID) :
    socket(*context),
    started(false),
    username(ID)
{
...
Peer::ptr Peer::create_peer(boost::asio::io_context *context, std::string ID)
{
    ptr peer(new Peer(context, ID));
    return peer;
}
